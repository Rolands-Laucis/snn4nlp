...
for sim_steps in 10 15 20 25 30; do
    ...
    python experiments/pos_task.py --input_mode "temporal" --epochs 5 --beta 0.95 --sim_steps "${sim_steps}" --output_file_prefix "var-sim_steps" --limit 100
done
for beta in 0.5 0.75 0.9 0.95 0.99; do
    echo "Running phase-0 pos_task with beta=${beta}"
    python experiments/pos_task.py --input_mode "temporal" --epochs 5 --beta "${beta}" --sim_steps 20 --output_file_prefix "var-beta" --limit 100
done